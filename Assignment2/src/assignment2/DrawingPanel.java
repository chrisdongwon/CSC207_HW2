...
/**
 * The DrawingPanel class provides a simple interface for drawing persistent images using a Graphics
 * object. An internal BufferedImage object is used to keep track of what has been drawn. A client
 * of the class simply constructs a DrawingPanel of a particular size and then draws on it with the
 * Graphics object, setting the background color if they so choose.
 * <p>
 * 
 * To ensure that the image is always displayed, a timer calls repaint at regular intervals.
 * <p>
 * 
 * This version of DrawingPanel also saves animated GIFs, though this is kind of hit-and-miss
 * because animated GIFs are pretty sucky (256 color limit, large file size, etc).
 * <p>
 * 
 * Recent features: - save zoomed images (2011/10/25) - window no longer moves when zoom changes
 * (2011/10/25) - grid lines (2011/10/11)
 * 
 * @author Marty Stepp
...
public final class DrawingPanel extends FileFilter
  ...
  private static final boolean SAVE_SCALED_IMAGES = true; // if true, when panel is zoomed, saves
                                                          // images at that zoom
                                                          // factor
  ...
  private static void checkAnimationSettings() {
    try {
      ...
      if (settingsFile.exists()) {
        ...
        System.out.println("***");
        System.out.println(
            "*** DrawingPanel saving animated GIF: " + new File(animationSaveFileName).getName());
        System.out.println("***");
        settingsFile.delete();
   ...
   * prop.equalsIgnoreCase("no") || prop.equalsIgnoreCase("0")); } catch (SecurityException e) { if
   * (DEBUG) System.out.println("Security exception when trying to read " + name); return false; } }
   */
  ...
  public DrawingPanel(int width, int height) {
    ...
    if (DEBUG)
      System.out.println("w=" + width + ",h=" + height + ",anim=" + isAnimated() + ",graph="
          + isGraphical() + ",save=" + shouldSave());
  ...
  public void save(String filename) throws IOException {
    ...
    if (SAVE_SCALED_IMAGES && currentZoom != 1) {
      BufferedImage zoomedImage =
          new BufferedImage(width * currentZoom, height * currentZoom, image.getType());
  ...
  private void initializeAnimation() {
     ...
     * FileOutputStream(System.getProperty(SAVE_PROPERTY)); } // encoder.startEncoding(stream); }
  ...
  private boolean readyToClose() {
    /*
     * if (isAnimated()) { // wait a little longer, in case animation is sleeping return
     * System.currentTimeMillis() > createTime + 5 * DELAY; } else { return
  ...
  private String saveAsHelper(String extension) {
    ...
    if (new File(filename).exists()
        && JOptionPane.showConfirmDialog(frame, "File exists.  Overwrite?", "Overwrite?",
  ...
  private int showOptionDialog(Frame parent, String title, String message, final String[] names) {
    ...
    // ((JComponent)
    // dialog.getContentPane()).setBorder(BorderFactory.createEmptyBorder(10, 10,
    // 10, 10));
  ...
  private class DiffImage extends JPanel implements ActionListener, ChangeListener {
    ...
    public void paintComponent(Graphics g) {
      ...
      if (image2 != null) {
        ...
        g2.setComposite(
            AlphaComposite.getInstance(AlphaComposite.SRC_ATOP, ((float) opacity) / 100));
  ...
  // ==============================================================================
  /**
   * Instances of this Gif89Frame subclass are constructed from RGB image info, either in the form
   * of an Image object or a pixel array.
   * <p>
   * There is an important restriction to note. It is only permissible to add DirectGif89Frame
   * objects to a Gif89Encoder constructed without an explicit color map. The GIF color table will
   * be automatically generated from pixel information.
   *
  ...
  class DirectGif89Frame extends Gif89Frame {
     ...
     * @exception IOException If the image is unencodable due to failure of pixel-grabbing.
     */
   ...
   * table. But that is usually what you want anyway, so as to avoid irregularities on some
   * displays. (So this is not really a limitation, but a "disciplinary feature" :) Another rather
   * more serious restriction is that the total number of RGB colors in a given input-batch mustn't
   * exceed 256. Obviously, there is an opening here for someone who would like to add a
   ...
   * should be able to make multiple calls to encode() on the same object, with or without
   * intervening frame additions or changes to options. But I haven't reviewed the code with such
   * usage in mind, much less tested it, so it's likely I overlooked something. (2) The encoder
   * classes aren't thread safe, so use caution in a context where access is shared by multiple
  ...
  class Gif89Encoder {
     ...
     *
     * @param colors Array of color values; no more than 256 colors will be read, since that's the
     *        limit for a GIF.
     */
     ...
     * @exception IOException If Gif89Frame can't be accommodated. This could happen if either (1)
     *            the aggregate cross-frame RGB color count exceeds 256, or (2) the Gif89Frame
     ...
     * @exception IOException If either (1) pixel-grabbing fails, (2) the aggregate cross-frame RGB
     *            color count exceeds 256, or (3) this encoder object was constructed with an
     ...
     * @param height Height of same.
     * @param ci_pixels Array of color-index pixels no less than width * height in length.
     * @exception IOException Actually, in the present implementation, there aren't any unchecked
     *            exceptions that can be thrown when adding an IndexGif89Frame <i>per se</i>. But I
     *            might add some pedantic check later, to justify the generality :)
     */
     ...
     *
     * @param index Index of the color that should be rendered as transparent, if any. A value of -1
     ...
     *
     * @param count Number of times to play sequence. Special value of 0 specifies indefinite
     *        looping. (Default: 1)
     */
    ...
    // ----------------------------------------------------------------------------
    /**
     * After adding your frame(s) and setting your options, simply call this method to write the GIF
     * to the passed stream. Multiple calls are permissible if for some reason that is useful to
   ...
   * encode a single static image and its associated "control" data. A Gif89Frame doesn't know or
   * care whether it is encoding one of the many animation frames in a GIF movie, or the single
   * bitmap in a "normal" GIF. (FYI, this design mirrors the encoded GIF structure.)
   * <p>
   ...
   * java.awt.Image object, but the lower-level IndexGif89Frame class offers advantages in
   * specialized circumstances. (Of course, in routine situations you might not explicitly
   * instantiate any frames at all, instead letting Gif89Encoder's convenience methods do the
   * honors.)
   * <p>
   ...
   * Gif89Frames, the reverse is NOT true. That is, even though the ultimate encoding of a
   * Gif89Frame may be affected by the context its parent encoder object provides, it retains its
   * original condition and can be reused in a different context.)
   * <p>
   * The core pixel-encoding code in this class was essentially lifted from Jef Poskanzer's
   * well-known <cite>Acme GifEncoder</cite>, so please see the <a href="../readme.txt">readme</a>
   * containing his notice.
   *
   * @version 0.90 beta (15-Jul-2000)
   * @author J. M. G. Elliott (tep@jmge.net)
   * @see Gif89Encoder
  ...
  abstract class Gif89Frame {
     ...
     *
     * @param p Coordinates of the frame's upper left corner in the display space. (Default: The
     ...
     *
     * @param interval Centiseconds to wait before displaying the subsequent frame. (Default: 0)
     */
     ...
     *
     * @param code One of the four int constants of the Gif89Frame.DM_* series. (Default: DM_LEAVE)
     */
  ...
  // ==============================================================================
  /**
   * Instances of this Gif89Frame subclass are constructed from bitmaps in the form of color-index
   * pixels, which accords with a GIF's native palettized color model. The class is useful when
   * complete control over a GIF's color palette is desired. It is also much more efficient when one
   * is using an algorithmic frame generator that isn't interested in RGB values (such as a cellular
   * automaton).
   * <p>
   * Objects of this class are normally added to a Gif89Encoder object that has been provided with
   * an explicit color table at construction. While you may also add them to "auto-map" encoders
   * without an exception being thrown, there obviously must be at least one DirectGif89Frame object
   * in the sequence so that a color table may be detected.
   *
   * @version 0.90 beta (15-Jul-2000)
   * @author J. M. G. Elliott (tep@jmge.net)
   * @see Gif89Encoder
  ...
  // ----------------------------------------------------------------------------
  /**
   * Write just the low bytes of a String. (This sucks, but the concept of an encoding seems
   * inapplicable to a binary file ID string. I would think flexibility is just what we don't want -
   * but then again, maybe I'm slow.)
   */
